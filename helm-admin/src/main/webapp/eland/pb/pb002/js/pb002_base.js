...
var pb002Base = {
    ...
    init: function (method) {
        //alert("暂时定位用：画图 pb002Base.init() 方法 带有回退，删除，修改。");
        var lon, lat, level;
    ...
    editBuild: function (obj, buildId) {
        //替换整个面板div
        var _this = $(obj);
        var urlData = "eland/pb/pb002/pb002-initYardInfo.gv?method=edit&buildId=" + buildId + "&prjCd=" + getPrjCd();
    ...
    viewMap: function () {
        var lon, lat, level;
        var strPoints = $("input[name=buildPosition]", navTab.getCurrentPanel()).val();
        var lonLatLevel = $("input[name=lonLatLevel]", navTab.getCurrentPanel()).val();
        var mapBounds = $("input[name=mapBounds]", navTab.getCurrentPanel()).val();
        level = 2; lon = 1425; lat = 700;
        yardView = new OpenLayers.Map({
            ...
            maxExtent: new OpenLayers.Bounds(580,-100,2243,1010),     //边界：left, bottom, right, top
            maxResolution: "auto",                                      //最大分辨率（maxResolution）
            numZoomLevels: 6,
    ...
    saveYardInfo: function (obj, method) {
        ...
        if (method == 'edit') {
            $pb002Basefrm = $("#pb002Base_edit_frm", navTab.getCurrentPanel());
        } else if (method == 'add') {
            $pb002Basefrm = $("#pb002Base_add_frm", navTab.getCurrentPanel());
        }
        if ($pb002Basefrm.valid()) {
            var url = getGlobalPathRoot() + "eland/pb/pb002/pb002-saveYardInfo.gv?method=" + method;
            var ajaxPacket = new AJAXPacket(url);

            //序列化表单
            var jsonData = $pb002Basefrm.serializeArray();
            jsonData.push({
                name: 'buildPoints',
                value: temp
            });
            ...
            core.ajax.sendPacketHtml(ajaxPacket, function (response) {
                ...
                if (isSuccess) {
                    ...
                    if (method == 'edit') {
                        ...
                        var url = "eland/pb/pb002/pb002-init.gv?regId=" + regId + "&buldId=" + buildId;
                        index.openTab(null, "pb002", url, "pb002", "院落信息");
                    }
                } else {
