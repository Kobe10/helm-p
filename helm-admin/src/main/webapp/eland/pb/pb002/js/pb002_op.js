var pb002Op = {
    ...
    openBuilding: function (buldId) {
        var url = "eland/pb/pb002/pb002-init.gv?buldId=" + buldId;
    ...
    deleteBuild: function (buildId) {
        alertMsg.confirm("删除院落将同时删除院落内的房产信息，你确定要删除吗？", {
            okCall: function () {
                ...
                // 提交界面展示
                packet.url = getGlobalPathRoot() + "eland/pb/pb001/pb001-deleteBuild.gv";
                // 处理删除
    ...
    exportTxAzF: function (buildId) {
        var url = getGlobalPathRoot() + "eland/pb/ph009/ph009-exportTxAzF.gv?buildId=" + buildId + "&prjCd=" + getPrjCd();
        window.open(url)
    },
    ...
    exportTtCb: function (buildId) {
        var url = getGlobalPathRoot() + "eland/pb/pb002/pb002-exportTtCb.gv?buildId=" + buildId + "&prjCd=" + getPrjCd();
        window.open(url)
    },
     ...
     * @param buildId 院落编号
     */
    browseBuildFj: function (buildId) {
        var url = getGlobalPathRoot() + "eland/ph/ph011/ph011-bldFjDialog.gv?buildId="
            + buildId + "&prjCd=" + getPrjCd();
        $.pdialog.open(url, "ph011-bldFj", "院落附件", {mask: true, height: 600, width: 800});
    },
    ...
    exportYardFj: function (buildId) {
        var url = getGlobalPathRoot() + "eland/pb/pb002/pb002-exportYardFj.gv?buildId=" + buildId + "&prjCd=" + getPrjCd();
        window.open(url);
    },
    ...
    exportBuildFj: function (buildId) {
        var url = url = getGlobalPathRoot() + "eland/pb/pb002/pb002-exportFj.gv?buildId=" + buildId + "&prjCd=" + getPrjCd();
        window.open(url)
    },
