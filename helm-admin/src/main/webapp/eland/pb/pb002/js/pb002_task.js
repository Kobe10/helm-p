var pb002Task = {
    ...
    initiateTask: function (procDefKey, buildId) {
        index.startWf(procDefKey, getPrjCd(), {
            busiKey: buildId
        });
    },
    ...
    cfmSendTask: function (buildId) {
        var url = getGlobalPathRoot() + "eland/pb/pb002/pb002-cfmSendTask.gv?prjCd=" + getPrjCd() + "&buildId=" + buildId;
    ...
    historyTask: function (buildId) {
        var url = getGlobalPathRoot() + "eland/pb/pb002/pb002-historyTask.gv?prjCd=" + getPrjCd() + "&buildId=" + buildId;
        $.pdialog.open(url, "pb002Task", "历史任务", {
            mask: true, max: false, width: 880, height: 650
            //close: pb002Task.faqirenwu
        });
    },
    ...
    completeTask: function (obj) {
        var url = getGlobalPathRoot() + "eland/pb/pb002/pb002-cfmCompleteTask.gv?prjCd=" + getPrjCd();
        var packet = new AJAXPacket(url);
     ...
     */
    submitMemo: function () {
        var $form = $("#_pb002Taskfrm_spat", navTab.getCurrentPanel());
        if ($form.valid()) {
            // 提交页面数据
