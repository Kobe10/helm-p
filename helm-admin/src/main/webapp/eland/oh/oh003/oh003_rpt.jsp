...
<div layoutH="57" id="oh003_rpt" style="border: 1px solid #e9e9e9;">
    <%--统计工作进展--%>
    <div class="js_sum" id="oh003_rpt_000" style="position: relative;">
        <span class="link js_item_reload" style="position: absolute; top: 10px;right: 20px; display: inline-block;">
            刷新
        </span>
        <c:set var="groupName" value="buyHsCtDate"/>
        <c:choose>
            <c:when test="${param.viewFilter  == 'f2'}">
                <c:set var="groupName" value="jsTime"/>
            </c:when>
        ...
        <form>
            ...
            <input type="hidden" name="xAttrs" value="NewHsInfo.${groupName},NewHsInfo.hsSt">
            <input type="hidden" name="xAttrFuncs" value="NewHsInfo.${groupName}|1|2">
            <input type="hidden" name="resultAttrsName" value="NewHsInfo.newHsId">
            <input type="hidden" name="resultAttrsType" value="2">
            <input type="hidden" name="orderAttrsName" value="NewHsInfo.hsSt,NewHsInfo.${groupName}">
            <input type="hidden" name="orderAttrsOrder" value="desc,desc">
        </form>
        <div class="js_sum_rslt"></div>
    </div>
    <div class="js_sum" id="oh003_rpt_001" style="position: relative;">
        <span class="link js_item_reload" style="position: absolute; top: 10px;right: 20px; display: inline-block;">
            刷新
        </span>
    ...
    <div class="js_sum" id="oh003_rpt_002" style="position: relative;">
        <span class="link js_item_reload" style="position: absolute; top: 10px;right: 20px; display: inline-block;">
            刷新
        </span>
...
<script type="text/javascript">
    ...
    $(function () {
        ...
        $("div.js_sum", navTab.getCurrentPanel()).delegate("span.js_item_reload", "click", function () {
            oh003_rpt.loadPic($(this).closest("div.js_sum"));
        });
        // 自动加载所有的图片
        $("div.js_sum", navTab.getCurrentPanel()).each(function () {
            oh003_rpt.loadPic(this);
        });
    });
