    ...
    <c:forEach items="${hsInfoList}" var="item" varStatus="varStatus">
        <tr>
            ...
            <c:choose>
                ...
                <c:otherwise>
                    <%--公房--%>
                    <td><oframe:name prjCd="${param.prjCd}" itemCd="FINISH_RESULT" value="${item.Row.procResult2}"/></td>
                    <td><span class="btn" style="color: #0000ff" onclick="ph007.viewTzsq('${hsId}');">退租结果</span></td>
                </c:otherwise>
            </c:choose>
        </tr>
    </c:forEach>
