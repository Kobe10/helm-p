...
<div class="panel" id="sys012Summary">
    ...
    <div>
        <form>
            <input type="hidden" name="oldEntityName" value="${group.GroupInfo.entityName}">
            <input type="hidden" name="oldEntityGroupId" value="${group.GroupInfo.entityGroupId}">
            <input type="hidden" name="entityGroupStatus" value="${group.GroupInfo.entityGroupStatus}">
            <input type="hidden" name="usePrjCd" value="${group.GroupInfo.prjCd}">
            <input type="hidden" name="nodeId" value="${id}">

            <table class="border">
                <tr>
                    <th><label>分组编号：</label></th>
                    <td>
                        <input type="text" name="entityGroupId" class="required"
                               value="${group.GroupInfo.entityGroupId}">
                    </td>
                    <th><label>归属实体：</label></th>
                    <td>
                        <input type="text" name="entityName" class="required" value="${group.GroupInfo.entityName}">
                    </td>
                    <th><label>中文名称：</label></th>
                    <td>
                        <input type="text" name="entityGroupNameCh" class="required"
                               value="${group.GroupInfo.entityGroupNameCh}">
                    </td>
                    <th><label>英文名称：</label></th>
                    <td>
                        <input type="text" name="entityGroupNameEn" class="required"
                               value="${group.GroupInfo.entityGroupNameEn}">
                    </td>
                </tr>
                <tr>
                    ...
                    <td colspan="7">
                        <input type="text" name="entityGroupDesc" class="required"
                               value="${group.GroupInfo.entityGroupDesc}">
                    </td>
                </tr>
                <tr>
                    <td colspan="8" align="center">
                        <span onclick="sys012.updateGroup('${id}','${pId}',this)" class="btn btn-primary">保存分组</span>
                            <span onclick="sys012.addView('group','${group.GroupInfo.entityName}','${pId}', '${group.GroupInfo.prjCd}')"
                                  class="btn btn-info marl5">
                                新建分组
                            </span>
                        <c:if test="${staffId == '0'}">
                            <span class="btn btn-warn marl5" onclick="sys012.deleteInfo('group');">删除分组</span>
                        </c:if>
                    </td>
                </tr>
            </table>
...
<div class="mart5" style="position: relative;">
    ...
    <div style="display: none;">
        <form id="sys012EditFrm" method="post">
            <%--分组查询条件--%>
            <input type="hidden" name="entityGroupId" value="${group.GroupInfo.entityGroupId}">
            <%--实体查询条件--%>
            <input type="hidden" name="entityName" value="${group.GroupInfo.entityName}">
            <%--自服务名称用于定位SQL--%>
