rhtTree = {
    ...
    initFullTree: function () {
        ...
        // 权限编码
        var rhtType = $("select[name=rhtType]", $.pdialog.getCurrent()).val();
        if (rhtType == "") {
            rhtType = "1";
        }
        // 角色编码
        var staffCode = $("input[name=staffCode]", $.pdialog.getCurrent()).val();
        if (staffCode == "") {
            return;
        }
        packet.url = getGlobalPathRoot() + "oframe/sysmg/sys002/sys002-rhtInfo.gv";
        packet.data.add("rhtType", rhtType);
        packet.data.add("prjCd", getPrjCd());
        packet.data.add("staffCode", staffCode);
...
//                console.log(treeJson);
    ...
    roleCheck: function (event, treeId, treeNode) {
        ...
        if (treeNode.getParentNode()) {
            rhtTree.checkAllParent(treeObj, treeNode.getParentNode(), false);
        }
    ...
    onConfim: function (obj) {
        ...
        packet.url = getGlobalPathRoot() + "oframe/sysmg/sys002/sys002-saveRht.gv";
        packet.data.add("prjCd", getPrjCd());
        packet.data.add("rhtType", rhtType);
