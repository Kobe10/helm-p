    ...
    function _generate(mirror, obj, patches, path) {
        ...
        for (var t = oldKeys.length - 1; t >= 0; t--) {
            ...
            if (obj.hasOwnProperty(key) && !(obj[key] === undefined && oldVal !== undefined && _isArray(obj) === false)) {
                ...
                if (typeof oldVal == "object" && oldVal != null && typeof newVal == "object" && newVal != null) {
                    ...
                    if (oldVal !== newVal) {
                        changed = true;
                        patches.push({ op: "replace", path: path + "/" + escapePathComponent(key), oldValue:deepClone(oldVal), newValue: deepClone(newVal) });
                    }
                }
            }
            else {
        ...
        for (var t = 0; t < newKeys.length; t++) {
            ...
            if (!mirror.hasOwnProperty(key) && obj[key] !== undefined) {
                patches.push({ op: "add", path: path + "/" + escapePathComponent(key), newValue: deepClone(obj[key]) });
            }
        }
    }
    var _isArray;
