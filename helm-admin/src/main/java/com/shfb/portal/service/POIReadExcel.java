...
public class POIReadExcel {
    ...
    public static String getExcelInfo(Workbook wb, boolean isWithStyle) {
        ...
        sb.append("<table style='border-collapse:collapse;' align='center' width='70%'>");
        Row row = null; // 兼容
        Cell cell = null; // 兼容
    ...
    private static String getCellValue(Cell cell) {
        ...
        switch (cell.getCellType()) {
            ...
            case Cell.CELL_TYPE_STRING:// String类型
                ...
                break;
            case HSSFCell.CELL_TYPE_FORMULA:// todo 判断取值类型，兼容公式计算，直接获取公式的值
                result = getCellFormatValue(cell);
//                try {
//                    result = cell.getStringCellValue();
//                } catch (IllegalStateException e) {
//                    // todo 数据格式，保留两位
////                    double valueT = cell.getNumericCellValue();
////                    DecimalFormat df = new DecimalFormat("#");
////                    result = df.format(valueT);
...
//                }
    ...
    private static String getCellFormatValue(Cell cell) {
        ...
        try {// 如果内容是数字，用String取值会报错，所以需要try块来处理
            fValue = cell.getStringCellValue();
        } catch (IllegalStateException e) {
            // todo 数据格式，保留两位
            fValue = String.valueOf(NumberUtil.getDoubleFromObj(cell.getNumericCellValue()));
            DecimalFormat df = new DecimalFormat("#");
            String fValue2 = df.format(NumberUtil.getDoubleFromObj(cell.getNumericCellValue()));
            // ① 取整后如果数值不同，说明小数点后有值，且不为0，默认保留两位 ||
            // ② 非常规类型 && 非文本类型  则默认都是数字类型（该处定值保留两位）
            if (NumberUtil.getDoubleFromObj(fValue).equals(NumberUtil.getDoubleFromObj(fValue2)) || (!StringUtil.isEqual(cellFormat, "General") && !StringUtil.isEqual(cellFormat, "@"))) {
                df = new DecimalFormat("#.00");
                fValue = df.format(NumberUtil.getDoubleFromObj(cell.getNumericCellValue()));
            }
        }
        return fValue;
    }
